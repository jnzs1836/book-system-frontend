...
export default class BooksList extends PureComponent {
    ...
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        ...
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <FormItem label="作者">
              {getFieldDecorator('author')(<Input style={{ width: '100%' }} />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="出版社">
              {getFieldDecorator('press')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="到期时间">
              {getFieldDecorator('due_date')(
                <DatePicker style={{ width: '100%' }} placeholder="请输入到期时间" />
              )}
            </FormItem>
          </Col>
        </Row>
        <Row>
          ...
          <Col md={8} sm={24}>
            <FormItem label="排序方法">
              {getFieldDecorator('sortWay')(
                <Select placeholder="请选择" style={{ width: '100%' }}>
                  <Option value="desc">降序</Option>
                  <Option value="incs">升序</Option>
                  {/*<Option value="2">已借阅</Option>*/}
                </Select>
              )}
            </FormItem>
  ...
  render() {
    const { book: { data }, dispatch, loading } = this.props;
    ...
    const action = record => {
      dispatch({
        ...
        .then(() => {
          dispatch(routerRedux.push('/book/manage'));
        });
    };
    const remove = record => {
      dispatch({
    ...
    const menu = (
      <Menu onClick={this.handleMenuClick} selectedKeys={[]}>
        ...
        <Menu.Item key="approval">删除</Menu.Item>
      </Menu>
    );
